 ...
 * ```
 */
export function RunTimelinePanel(props: RunTimelinePanelProps): React.ReactElement {
                        ...
                        <div className="mt-3 space-y-2">
                            {events.map((event) => (
                                <div key={event.id} className="grid grid-cols-[120px_1fr_auto] gap-3 text-xs">
                                    <span
                                        className="text-zinc-500">{new Date(event.timestamp).toLocaleTimeString()}</span>
                                    <span className="text-zinc-200">{event.label}</span>
                                    <span
                                        className="text-emerald-300">{event.durationMs ? `${event.durationMs}ms` : '•'}</span>
                                </div>
                            ))}
                        </div>
                    </article>
