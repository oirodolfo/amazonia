 ...
 * ```
 */
export function TerminalTabsBar(props: TerminalTabsBarProps): JSX.Element {
  return (
    <div className="flex h-12 items-center gap-2 border-b border-emerald-400/10 bg-zinc-950/80 px-3">
      {props.sessions.map((session) => (
        <div key={session.id} className={[
          ...
          <button type="button" onClick={() => props.onRestart(session.id)} className="text-emerald-300">↻</button>
