...
export function WorkbenchLayout(): React.ReactElement {
    ...
    async function runAction(action: WorkspaceAction): Promise<void> {
        ...
        const terminalPlan = createTerminalCommandPlan(
            action,
            plan.run.startedAtIso,
        );
        const tab = orchestratorRef.current.create({
            id: plan.run.id,
            title: terminalPlan.title,
            cwd: terminalPlan.cwd,
            command: terminalPlan.command,
            status: "running",
        ...
        await bridge.spawnTerminal({
            ...
            cwd: terminalPlan.cwd,
            command: terminalPlan.command,
            cols: terminalPlan.cols,
            rows: terminalPlan.rows,
        });
    }
    ...
    return (
        <div
            className="h-screen bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,.18),transparent_34%),radial-gradient(circle_at_bottom_right,rgba(14,165,233,.10),transparent_30%),linear-gradient(135deg,#05070a,#09090b_45%,#06110d)] text-zinc-100">
            <Group
            ...
            >
                ...
                <Separator className="w-1 bg-white/5 transition hover:bg-emerald-400/40"/>
                <Panel defaultSize={52} minSize={32}>
                    <main className="grid h-full grid-rows-[auto_minmax(0,1fr)_auto] gap-3 p-4">
                        <TerminalTabsBar
                            ...
                            onCloseTab={(tabId) => {
                                void bridge.killTerminal(tabId);
                                orchestratorRef.current.remove(tabId);
                                setTabs(orchestratorRef.current.toTabs());
                            }}
                        />
