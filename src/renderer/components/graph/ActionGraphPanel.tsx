...
export function ActionGraphPanel({workspace}: ActionGraphPanelProps): React.ReactElement {
    if (workspace === null) {
        return <div
            className="rounded-2xl border border-white/10 bg-black/20 p-4 text-xs text-zinc-500">{t('graph.empty')}</div>;
    }
                ...
                {workspace.packages.slice(0, 6).map((workspacePackage) => {
                    ...
                    return (
                        <div key={workspacePackage.id} className="rounded-xl border border-white/5 bg-zinc-950/60 p-3">
                            <div
                                className="mb-2 truncate text-xs font-medium text-zinc-200">{workspacePackage.name}</div>
                            <div className="flex flex-wrap gap-1.5">
                                {actions.map((action) => <span key={action.id}
                                                               className="rounded-full bg-emerald-400/10 px-2 py-1 text-[10px] text-emerald-100">{action.label}</span>)}
                            </div>
                        </div>
                    );
                })}
