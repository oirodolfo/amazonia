  ...
  return (
    <aside className="flex h-full flex-col gap-4 border-l border-white/10 bg-black/30 p-4">
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-auto pr-1">
        {cards.length === 0 ? <div className="rounded-2xl border border-dashed border-white/10 p-4 text-sm text-zinc-500">{t('output.noRuns')}</div> : null}
        {cards.map((card) => (
          <Card key={card.id} className="p-4">
            ...
            <div className="space-y-2">
              {card.diagnostics.slice(0, 8).map((diagnostic, index) => (
                <div key={`${card.id}-${index}`} className="rounded-xl border border-white/5 bg-zinc-950/70 p-2 text-xs text-zinc-300">
                  <div className="flex items-center gap-2">
                    {diagnostic.level === 'error' ? <XCircle size={13} className="text-red-300" /> : diagnostic.level === 'warning' ? <AlertTriangle size={13} className="text-amber-300" /> : <CheckCircle2 size={13} className="text-emerald-300" />}
                    <span className="line-clamp-2">{diagnostic.message}</span>
                  </div>
                  {diagnostic.filePath !== undefined ? <div className="mt-1 flex items-center gap-1 text-[10px] text-emerald-200"><ExternalLink size={11} />{diagnostic.filePath}:{diagnostic.line ?? 1}</div> : null}
                </div>
              ))}
            </div>
          </Card>
...
function Metric({ label, value }: { readonly label: string; readonly value: string }): React.ReactElement {
  return <div className="rounded-xl bg-white/[0.03] p-2"><div className="text-[10px] uppercase tracking-wider text-zinc-600">{label}</div><div className="text-zinc-200">{value}</div></div>;
}
