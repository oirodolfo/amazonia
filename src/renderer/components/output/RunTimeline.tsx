...
export function RunTimeline({cards}: RunTimelineProps): React.ReactElement {
    return (
        <section
            className="rounded-2xl border border-emerald-300/10 bg-black/20 p-3 shadow-[0_0_35px_rgba(16,185,129,.08)]">
                    ...
                    <div key={card.id} className="grid grid-cols-[72px_1fr_auto] items-center gap-2 text-xs">
                        ...
                        <div className="h-2 overflow-hidden rounded-full bg-zinc-900">
                            <div className="h-full rounded-full bg-emerald-400/70"
                                 style={{width: `${Math.min(100, Math.max(8, (card.durationMs ?? 80) / 30))}%`}}/>
                        </div>
                        <span
                            className={card.status === 'success' ? 'text-emerald-300' : card.status === 'failed' ? 'text-red-300' : 'text-zinc-400'}>{card.status}</span>
                    </div>
                ))}
            </div>
        </section>
