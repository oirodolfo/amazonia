...
export function CommandPalette({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/60 pt-24 backdrop-blur-sm"
            ...
            <Command
                className="w-[720px] overflow-hidden rounded-2xl border border-emerald-300/20 bg-zinc-950 shadow-[0_0_80px_rgba(16,185,129,.18)]"
                onClick={(event) => event.stopPropagation()}>
                <CommandInput
                    className="w-full border-b border-white/10 bg-transparent px-4 py-4 text-sm text-white outline-none placeholder:text-zinc-600"
                    ...
                    <CommandGroup heading={t('command.actions')}>
                        {workspace?.actions.map((action) => (
                            <CommandItem key={action.id}
                                         ...
                                         className="cursor-pointer rounded-xl px-3 py-2 text-sm text-zinc-300 data-[selected=true]:bg-emerald-400/10 data-[selected=true]:text-emerald-100">
                                <span>{action.packageName}</span><span
                                className="mx-2 text-zinc-600">›</span><span>{action.label}</span>
                            </CommandItem>
                        ))}
                    </CommandGroup>
                </CommandList>
