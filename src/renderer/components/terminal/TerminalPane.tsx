...
export function TerminalPane({activeTab}: TerminalPaneProps): React.ReactElement {
    ...
    if (activeTab === null) {
        return <div
            className="flex h-full items-center justify-center text-sm text-zinc-500">{t('terminal.emptyTitle')}</div>;
    }

    return <div ref={containerRef}
                className="h-full w-full overflow-hidden rounded-xl border border-white/10 bg-[#05070a] p-2 shadow-inner shadow-emerald-950/20"/>;
}
