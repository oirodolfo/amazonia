...
export function RunInspectorPanel(props: RunInspectorPanelProps): React.ReactElement {
    ...
    return (
        <section
            className="rounded-2xl border border-emerald-400/15 bg-black/25 p-3 shadow-[0_0_30px_rgba(16,185,129,.08)]">
            ...
            <div className="space-y-2">
                {sortedCards.length === 0 ?
                    <p className="text-xs text-zinc-500">No runs yet. Start an action and the forest will light
                        up.</p> : null}
                    ...
                    return (
                        <article key={card.id} className="rounded-xl border border-white/10 bg-white/[0.03] p-3">
                            <div className="flex items-center justify-between gap-3">
                                ...
                                <span
                                    className="rounded-full bg-emerald-400/10 px-2 py-0.5 text-[10px] text-emerald-100">{health.score}</span>
                            </div>
