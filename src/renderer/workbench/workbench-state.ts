...
export function workbenchReducer(state: WorkbenchState, event: WorkbenchEvent): WorkbenchState {
  switch (event.type) {
    ...
    case 'terminal.session.upserted':
      return { ...state, terminalTabs: terminalTabsReducer(state.terminalTabs, { type: 'session.upserted', session: event.session }) };
    case 'terminal.session.closed':
      return { ...state, terminalTabs: terminalTabsReducer(state.terminalTabs, { type: 'session.closed', sessionId: event.sessionId }) };
    case 'terminal.session.activeChanged':
      return { ...state, terminalTabs: terminalTabsReducer(state.terminalTabs, { type: 'session.activeChanged', sessionId: event.sessionId }) };
