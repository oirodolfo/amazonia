    ...
    <div className="h-screen w-screen overflow-hidden bg-[#030706] text-zinc-100">
      <Group orientation="horizontal">
        <Panel minSize={18} defaultSize={24}>
          <ActionSidebar groups={props.actionGroups} state={props.sidebarState} onRunAction={props.onRunAction} onQueryChange={props.onQueryChange} t={props.t} />
        </Panel>
        <Separator className="w-1 bg-emerald-400/10 hover:bg-emerald-400/30" />
        <Panel minSize={35} defaultSize={50}>
          <main className="flex h-full flex-col">
            <TerminalTabsBar sessions={props.terminalTabs.sessions} activeSessionId={props.terminalTabs.activeSessionId} onClose={props.onCloseTerminal} onRestart={props.onRestartTerminal} />
            <div className="flex-1 p-3">
              {activeSession ? (
                <XtermTerminalView sessionId={activeSession.id} onInput={props.onTerminalInput} onResize={props.onTerminalResize} />
              ) : (
                <div className="flex h-full items-center justify-center rounded-3xl border border-dashed border-emerald-400/20 bg-zinc-950/60 text-sm text-zinc-500">
        ...
        <Panel minSize={20} defaultSize={26}>
          <aside className="h-full overflow-auto border-l border-emerald-400/10 bg-zinc-950/70 p-3">
            ...
            <div className="space-y-3">{props.outputCards.map((card) => <FriendlyOutputCard key={card.id} card={card} />)}</div>
          </aside>
        </Panel>
      </Group>
    </div>
