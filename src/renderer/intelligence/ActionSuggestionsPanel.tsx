 ...
 * ```
 */
export function ActionSuggestionsPanel(props: ActionSuggestionsPanelProps): JSX.Element | null {
  if (props.suggestions.length === 0) {
    return null;
  }

  return (
    <section className="rounded-3xl border border-emerald-400/10 bg-zinc-950/80 p-4 shadow-[0_0_40px_rgba(16,185,129,0.08)]">
      ...
      <div className="mt-3 space-y-2">
        {props.suggestions.map((suggestion) => (
          ...
          >
            ...
            <span className="mt-2 block text-[11px] text-emerald-300/80">{suggestion.reasons.join(' · ')}</span>
          </button>
        ))}
      </div>
    </section>
