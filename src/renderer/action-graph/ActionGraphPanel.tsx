    ...
    <section className="rounded-3xl border border-emerald-400/10 bg-zinc-950 p-4">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        {actionNodes.slice(0, 24).map((node) => (
          <button key={node.id} type="button" className="rounded-2xl border border-emerald-400/10 bg-emerald-400/5 px-3 py-2 text-left text-xs text-emerald-50 hover:bg-emerald-400/10" onClick={() => props.onRunAction(node.id)}>
            {node.label}
          </button>
        ))}
      </div>
