...
export function PackageHoverCard({ workspacePackage, actions, children }: PackageHoverCardProps): React.ReactElement {
  ...
  return (
    <HoverCard.Root openDelay={180} closeDelay={100}>
      ...
      <HoverCard.Portal>
        <HoverCard.Content side="right" align="start" sideOffset={14} className="z-50 w-80 rounded-2xl border border-emerald-300/20 bg-zinc-950/95 p-4 text-zinc-100 shadow-[0_0_70px_rgba(16,185,129,.18)] backdrop-blur-xl">
          <div className="mb-3">
          ...
          <div className="space-y-1">
            {sortedActions.slice(0, 10).map((action) => (
              <div key={action.id} className="flex items-center gap-2 rounded-lg bg-white/[0.03] px-2 py-1.5 text-xs">
