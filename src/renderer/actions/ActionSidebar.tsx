...
export function ActionSidebar({ workspace, onOpenWorkspace, onRunAction, onFocusPackage }: ActionSidebarProps): React.ReactElement {
  ...
  return (
    <aside className="flex h-full flex-col gap-4 border-r border-white/10 bg-black/30 p-4">
      ...
      <label className="flex items-center gap-2 rounded-xl border border-white/10 bg-zinc-950 px-3 py-2 text-sm text-zinc-300">
        <Search size={15} />
        <input className="w-full bg-transparent outline-none placeholder:text-zinc-600" placeholder={t('sidebar.searchPlaceholder')} value={query} onChange={(event) => setQuery(event.target.value)} />
      </label>
      <div className="min-h-0 flex-1 space-y-3 overflow-auto pr-1">
        {workspace?.packages.map((workspacePackage) => {
          ...
          return (
            <PackageHoverCard key={workspacePackage.id} workspacePackage={workspacePackage} actions={packageActions}>
              <Card className="p-3" onMouseEnter={() => onFocusPackage(workspacePackage)}>
              <div className="mb-2 flex items-center justify-between gap-2">
                <div className="min-w-0">
                  <div className="truncate text-sm font-medium text-zinc-100">{workspacePackage.name}</div>
                  <div className="truncate text-xs text-zinc-500">{workspacePackage.relativePath}</div>
                </div>
                <span className="rounded-full bg-emerald-400/10 px-2 py-1 text-[10px] text-emerald-200">{packageActions.length}</span>
              </div>
              <div className="space-y-1">
                {packageActions.slice(0, 8).map((action) => (
                  <button key={action.id} onClick={() => onRunAction(action)} className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left text-xs text-zinc-300 transition hover:bg-emerald-400/10 hover:text-emerald-100">
                    <Play size={12} />
