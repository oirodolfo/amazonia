...
export function VividDiagnosticHoverCard(props: VividDiagnosticHoverCardProps): JSX.Element {
  ...
  return (
    <PremiumCard tone={diagnostic.accentTone} eyebrow={diagnostic.severity} title={diagnostic.title} className="w-[28rem]">
      ...
      <div className="mt-4 rounded-2xl border border-white/10 bg-black/45 p-3 font-mono text-xs">
        {diagnostic.previewLines.map((line, index) => (
          <div key={`${line}-${index}`} className="grid grid-cols-[2rem_1fr] gap-3">
            <span className="select-none text-zinc-600">{diagnostic.line ? diagnostic.line + index : index + 1}</span>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {diagnostic.actions.map((action) => (
          <button key={action.id} type="button" className="rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs text-zinc-200 hover:bg-white/10" onClick={() => props.onAction?.(diagnostic.id, action.id)}>
            {action.label}
