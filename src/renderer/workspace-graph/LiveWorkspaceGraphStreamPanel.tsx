      ...
      <div className="relative min-h-80 overflow-hidden rounded-3xl border border-violet-400/10 bg-black/35 p-4">
        <div className="relative grid grid-cols-2 gap-3 md:grid-cols-3">
          {props.graph.nodes.slice(0, 30).map((node) => (
            <button key={node.id} type="button" className="rounded-2xl border border-violet-400/10 bg-zinc-950/80 px-3 py-3 text-left transition hover:border-violet-300/30 hover:bg-violet-400/10" onClick={() => props.onSelectNode?.(node.id)}>
