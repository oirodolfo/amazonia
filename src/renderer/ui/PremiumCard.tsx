    ...
    <section className={cx('rounded-[1.75rem] border p-4 backdrop-blur-xl transition duration-300', tone.panel, tone.border, tone.glow, props.className)}>
      {props.eyebrow || props.title ? (
        <header className="mb-4">
          {props.eyebrow ? <p className={cx('text-xs uppercase tracking-[0.28em]', tone.accent)}>{props.eyebrow}</p> : null}
          {props.title ? <h2 className={cx('mt-1 text-lg font-semibold', tone.text)}>{props.title}</h2> : null}
        </header>
      ) : null}
      {props.children}
    </section>
