...
export function FriendlyOutputCard(props: FriendlyOutputCardProps): React.Element {
    ...
    return (
        <article
            className="rounded-2xl border border-emerald-400/10 bg-zinc-900/70 p-4 shadow-[0_0_30px_rgba(16,185,129,0.05)]">
            ...
            <div className="mt-4 space-y-2">
                {card.summary.issues.map((issue, index) => (
                    <div key={`${issue.message}-${index}`}
                        ...
                        <div
                            className={issue.severity === 'error' ? 'text-red-300' : 'text-amber-300'}>{issue.severity.toUpperCase()}</div>
                        <p className="mt-1 text-zinc-300">{issue.message}</p>
                        {issue.file ?
                            <p className="mt-2 font-mono text-emerald-300">{issue.file}:{issue.line ?? 1}:{issue.column ?? 1}</p> : null}
                    </div>
                ))}
            </div>
