                ...
                {sections.map((section) => {
                    ...
                    return (
                        <article key={section.id}
                                 className="overflow-hidden rounded-2xl border border-emerald-400/10 bg-black/35">
                            <button type="button"
                                    className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left"
                                    onClick={() => setFoldedIds((current) => toggleFoldedSection(current, section.id))}>
                <span>
                  ...
                  <span className="ml-3 text-xs text-zinc-500">{section.lines.length} lines</span>
                                ...
                                <span
                                    className="rounded-full bg-zinc-700/40 px-2 py-1 text-xs text-zinc-300">{section.severity}</span>
                            </button>

                            {!isFolded ? <pre
                                className="max-h-96 overflow-auto border-t border-emerald-400/10 px-4 py-3 text-xs text-zinc-300">{section.lines.join('\n')}</pre> : null}
                        </article>
                    );
                })}
            </div>
