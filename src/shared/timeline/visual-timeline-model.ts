...
export function buildVisualTimelineModel(events: readonly RunTimelineEvent[]): VisualTimelineModel {
    ...
    return {
        ...
        endedAt,
        durationMs,
        lanes: Object.entries(grouped).map(([runId, runEvents]) => createTimelineLane(runId, runEvents, startedAt, durationMs)).sort((left, right) => left.startedAt - right.startedAt)
    };
}
...
function createTimelineLane(runId: string, events: readonly RunTimelineEvent[], globalStart: number, globalDuration: number): TimelineLane {
    ...
    return {
        runId,
        label: resolveLaneLabel(events),
        startedAt,
        endedAt,
        durationMs,
        events,
        severity: resolveTimelineSeverity(events),
        leftPercent: Number((((startedAt - globalStart) / globalDuration) * 100).toFixed(3)),
        widthPercent: Number(Math.max(1, (durationMs / globalDuration) * 100).toFixed(3))
    };
}
