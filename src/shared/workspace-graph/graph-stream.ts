...
export function reduceGraphStream(state: GraphStreamState, event: GraphStreamEvent): GraphStreamState {
  switch (event.type) {
    ...
    case 'node.upserted':
      return { graph: { ...state.graph, nodes: upsertById(state.graph.nodes, event.node) }, eventCount: state.eventCount + 1 };
    case 'edge.upserted':
      return { graph: { ...state.graph, edges: upsertById(state.graph.edges, event.edge) }, eventCount: state.eventCount + 1 };
    case 'node.removed':
      return {
        graph: {
