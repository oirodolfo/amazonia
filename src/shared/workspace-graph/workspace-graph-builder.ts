...
export function buildWorkspaceGraph(options: BuildWorkspaceGraphOptions): WorkspaceGraph {
    ...
    nodes.set(rootId, {
        ...
        kind: 'root',
        weight: options.groups.length,
        metadata: {}
    });
    for (const group of options.groups) {
        nodes.set(group.packageId, {
            id: group.packageId,
            label: group.packageName,
            kind: 'package',
            weight: group.actions.length,
            metadata: {path: group.packagePath, tools: group.detectedTools}
        });
    ...
    return {
        nodes: [...nodes.values()].sort((left, right) => sortNode(left) - sortNode(right) || left.label.localeCompare(right.label)),
