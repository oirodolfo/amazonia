...
export function validateClientMessage(message: unknown): ProtocolValidationResult {
  ...
  if (typeof message.requestId !== 'string' || message.requestId.length === 0) return { valid: false, reason: 'Request id is required.' };

  if (message.type === 'workspace:scan') return typeof message.rootPath === 'string' ? { valid: true } : { valid: false, reason: 'Workspace root path is required.' };
  if (message.type === 'terminal:spawn') return isRecord(message.request) && typeof message.request.tabId === 'string' ? { valid: true } : { valid: false, reason: 'Terminal spawn request is invalid.' };
  if (message.type === 'terminal:input') return isRecord(message.message) && typeof message.message.tabId === 'string' ? { valid: true } : { valid: false, reason: 'Terminal input message is invalid.' };
  if (message.type === 'terminal:resize') return isRecord(message.message) && typeof message.message.cols === 'number' ? { valid: true } : { valid: false, reason: 'Terminal resize message is invalid.' };
  if (message.type === 'terminal:kill') return typeof message.tabId === 'string' ? { valid: true } : { valid: false, reason: 'Terminal kill tab id is required.' };
